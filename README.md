...
제 사이트에 처음 들어오면 저에대한 간단한 브리핑들을 부담없이 재밌는 인터렉션과 함께 보여드리도록 하고 싶습니다. 그러기 위해서 Home 이라는 메뉴를 개설하여 이것을 맨 처음에 보여지도록 했습니다. 여기서는 이력서의 Summary 항목을 가져와서 뿌리려고 합니다. 그 후 About 메뉴를 만들어 이곳에 나머지 이력서 정보를 긁어오는 방식을 취할 것입니다.
